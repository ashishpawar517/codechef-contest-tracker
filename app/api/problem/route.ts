...
export async function GET(request: NextRequest) {
  ...
  if (!problemCode) {
    return NextResponse.json({ error: "Problem code is required" }, { status: 400 })
  }

  try {
    const response = await fetch(`https://www.codechef.com/api/contests/PRACTICE/problems/${problemCode}`)
