...
export default function ContestProblems({ division, contestsRange, loading, setLoading }: ContestProblemsProps) {
  const [contests, setContests] = useState<{ [key: string]: Problem[] }>({})
  const [problemDetails, setProblemDetails] = useState<{ [key: string]: ProblemDetails }>({})
  ...
  useEffect(() => {
    const fetchContests = async () => {
      ...
      for (let contestNum = contestsRange.start; contestNum >= contestsRange.end; contestNum--) {
        try {
          ...
          console.error(`Error fetching contest ${contestNum}${division}:`, error)
        }
      }
    ...
    const fetchAllProblemDetails = async () => {
      ...
      for (const contestCode in contests) {
        for (const problem of contests[contestCode]) {
          ...
          } else if (!problemDetails[problem.code] && !detailsToFetch.includes(problem.code)) {
      ...
      for (const problemCode of detailsToFetch) {
        try {
          ...
          if (problemDetail) {
            ...
            localStorage.setItem(`problem-${problemCode}`, JSON.stringify(problemDetail))
          }
        } catch (error) {
          console.error(`Error fetching details for problem ${problemCode}:`, error)
        }
      }
    }
  ...
  if (Object.keys(contests).length === 0) {
    return <div className="text-center py-12 text-gray-500">No contests found for this division.</div>
  }
          ...
          {Object.entries(contests).map(([contestCode, problems], index) => (
            <tr key={contestCode}>
              ...
              {["A", "B", "C", "D", "E", "F", "G"].map((letter) => {
                ...
                const problem = problems.find((p, idx) => idx === letter.charCodeAt(0) - 65)
